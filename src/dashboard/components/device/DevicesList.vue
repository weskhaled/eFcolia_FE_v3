...
import { selectedClient, sideCollapsed, currentUser } from '~/common/stores'
...
<template>
  ...
  <div v-else-if="(!devicesLoading && !devicesCount)"
  ...
  <div v-infinite-scroll="[() => emit('onLoadMore'), { 'distance': 160 }]" v-bind="containerProps"
    ...
    <div
      ...
      <div class="flex items-center"
        ...
        <div v-if="!sideCollapsed" class="flex flex-grow items-center mr-auto">
          <a-input-search v-model:value="search" allow-clear placeholder="input search loading with enterButton"
        ...
        <div>
          <a-dropdown>
            ...
            <template #overlay>
              <a-menu>
                <a-menu-item :disabled="!hasPermission(currentUser.permissions, 'device', 'n')"
    ...
    <div v-bind="wrapperProps">
      <div v-for="{ index, data: item } in list" :id="`device-id-${item.id}`" :key="index" draggable="true"
        ...
        <template v-if="sideCollapsed">
          <a-tooltip :color="item.gprsstate === 1 ? 'green' : 'red'" placement="right">
            ...
            <div
              ...
              <a-badge :text="false" :status="item.gprsstate === 1 ? 'success' : 'error'"
        ...
        <template v-else>
          <DeviceCard :device="item" @click="emit('deviceClicked', item)" @show-details="emit('showDetails', item)"
