  ...
  <a-layout-header :theme="isDark"
    ...
    <div class="flex flex-grow px-3 w-auto h-full">
      <div
        ...
        <a-tree-select v-model:value="selectedClient" :loading="!treeDataClients"
    ...
    <div class="ml-auto flex items-center h-full px-3">
      ...
      <a-dropdown class="md:mx-2">
        <a-button type="link" :loading="!currentUser"
      ...
      <div class="hidden sm:flex">
        <a-dropdown class="mr-0">
          <a-button type="link" class="!flex items-center leading-12 !px-0.5 dark:text-white dark:hover:text-light-800"
            @click.prevent>
          ...
          <template #overlay>
            <a-menu class="max-h-55 overflow-y-auto">
              <a-menu-item v-for="availableLocale in availableLocales" :key="availableLocale" class="min-w-25">
                <a href="javascript:;" class="flex items-center text-md pr-2 text-left leading-8"
